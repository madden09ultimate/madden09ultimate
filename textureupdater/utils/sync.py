                    ...
                    if args.user_choice in ('full_scan', 'PY_VAR2') or commit_date > last_run_date:
                        ...
                        if files_response.status_code == 200:
                            ...
                            for file_info in files_info:
                                ...
                                if '- 1 2 user-customs' in relative_path:
                                    starter_line(file_status, relative_path)
                                    terminal_text.insert(tk.END, f"    Skipping file with '- 1 2 user-customs' in its path.\n")
                                    scroll_terminal()
                                    continue
								
                                if '- 1 1 JSGME' in relative_path:
                                    starter_line(file_status, relative_path)
                                    terminal_text.insert(tk.END, f"    Skipping file with '- 1 1 JSGME' in its path.\n")
                                    scroll_terminal()
                                    continue
